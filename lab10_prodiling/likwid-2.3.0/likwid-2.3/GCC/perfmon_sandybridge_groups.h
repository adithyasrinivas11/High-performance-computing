...
perfmon_printDerivedMetricsSandybridge(PerfmonGroup group)
{
    ...
    switch ( group ) 
    {
        ...
        case ENERGY:
            ...
            bstrListAdd(fc,2,Runtime rdtsc [s]);
            ...
            for(threadId=0; threadId < perfmon_numThreads; threadId++)
            {
                ...
                tableData.rows[1].value[threadId] = time ;
                stat[1][0] += (double) tableData.rows[1].value[threadId];
                stat[1][1] =  MAX(stat[1][1],(double) tableData.rows[1].value[threadId]);
                stat[1][2] =  MIN(stat[1][2],(double) tableData.rows[1].value[threadId]);
                ...
                tableData.rows[4].value[threadId] = perfmon_getResult(threadId,"PWR0") ;
                stat[4][0] += (double) tableData.rows[4].value[threadId];
                stat[4][1] =  MAX(stat[4][1],(double) tableData.rows[4].value[threadId]);
                stat[4][2] =  MIN(stat[4][2],(double) tableData.rows[4].value[threadId]);

                tableData.rows[5].value[threadId] = perfmon_getResult(threadId,"PWR0")/time;
                stat[5][0] += (double) tableData.rows[5].value[threadId];
                stat[5][1] =  MAX(stat[5][1],(double) tableData.rows[5].value[threadId]);
                stat[5][2] =  MIN(stat[5][2],(double) tableData.rows[5].value[threadId]);
        ...
        case FLOPS_DP:
            ...
            for(threadId=0; threadId < perfmon_numThreads; threadId++)
            {
                ...
                tableData.rows[4].value[threadId] = 1.0E-06*(perfmon_getResult(threadId,"PMC0")*2.0+perfmon_getResult(threadId,"PMC1"))/time;
                stat[4][0] += (double) tableData.rows[4].value[threadId];
                stat[4][1] =  MAX(stat[4][1],(double) tableData.rows[4].value[threadId]);
                stat[4][2] =  MIN(stat[4][2],(double) tableData.rows[4].value[threadId]);

                tableData.rows[5].value[threadId] = 1.0E-06*perfmon_getResult(threadId,"PMC0")/time;
                stat[5][0] += (double) tableData.rows[5].value[threadId];
                stat[5][1] =  MAX(stat[5][1],(double) tableData.rows[5].value[threadId]);
                stat[5][2] =  MIN(stat[5][2],(double) tableData.rows[5].value[threadId]);

                tableData.rows[6].value[threadId] = 1.0E-06*perfmon_getResult(threadId,"PMC1")/time;
...
perfmon_logDerivedMetricsSandybridge(PerfmonGroup group, double time,double timeStamp)
{
    ...
    switch ( group ) 
    {
        ...
        case ENERGY:
                        ...
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            printf(" %e  ",perfmon_getResult(threadId,"PWR0") );
                        }
                        printf("\n");
                        ...
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            printf(" %e  ",perfmon_getResult(threadId,"PWR0")/time);
                        }
                        printf("\n");
...
-\n\
SandyBridge implements the new RAPL interface. This interface enables to monitor the \n\
consumed energy on a package (socket) level.\n\
\n\
"},
